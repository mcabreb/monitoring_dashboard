...
- [x] Create Bluetooth devices section (AC: 3, 4, 5)
  - [ ] Add scrollable list of Bluetooth devices
  - [ ] Display type icon: 🎧 (headphones), 🖱️ (mouse), ⌨️ (keyboard), 🎮 (gamepad), 📱 (other)
  - [ ] Display device name
  - [ ] Display battery bar or "Battery: N/A"
  - [ ] Show "No Bluetooth devices connected" when list is empty
- [x] Implement update method (AC: 1, 2, 3)
...
- [x] Implement LED status indicator (AC: 7)
...
- [x] Integrate with app refresh loop
...
class DevicesPanel(BasePanel):
    ...
    def compose(self):
        yield Container(
            Static("🔋 Laptop Battery", id="battery-header"),
        ...
        yield Container(
            Static("📶 Bluetooth Devices", id="bt-header"),
